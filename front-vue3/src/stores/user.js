...
// import {extend} from '@/helpers/tools'
...
const useUserStore = defineStore('User', {
    ...
    actions: {
        ...
        filterAsyncRoute(asnycRoute, userRouters, routersArr = []) {
            ...
            for (const item of asnycRoute) {
                if (item.children && item.children.length > 0) {
                    const ret = this.filterAsyncRoute(item.children, userRouters, [])
                    if (ret) {
                        item.children = ret
        ...
        addUserRote(userRouters) {
            ...
            const userAsyncRoute = this.filterAsyncRoute(cloneDeep(asnycRoute), userRouters)
