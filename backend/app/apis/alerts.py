...
@alert_bp.route('/<string:alert_type>/<int:alert_id>', methods=['DELETE'])  # ❗️只接受 DELETE 方法
@token_required
def delete_alert_by_id(alert_type, alert_id):
    """
    根据类型和ID删除一个告警记录
    """
    try:
        ...
        if alert_type == 'normal':
            ...
            target_model = AbnormalEvent
        else:
            return jsonify(success=False, message="无效的告警类型"), 400

        # .get_or_404 会在找不到记录时自动返回 404 Not Found，很方便
        alert_to_delete = target_model.query.get_or_404(alert_id)

        # 在这里可以加上权限检查，比如只有管理员才能删除

        db.session.delete(alert_to_delete)
        db.session.commit()

        return jsonify(success=True, message="告警删除成功"), 200

    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"删除告警 {alert_type}-{alert_id} 时出错: {e}", exc_info=True)
